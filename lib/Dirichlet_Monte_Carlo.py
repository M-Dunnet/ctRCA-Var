...
class Variant:
	"""Represents a single SNP variant with relevant attributes."""
...
class VariantAnalyzer:
	...
	def __init__(self, test_file, ctrl_file, nn=500000):
	...
	def filter_variants(self):	#TODO Fix this so CONTIG_POS matches VARIANT LOCATIONS. // fix this method
		...
		variant_locations = self.test_file["Contig"].astype(str) + "_" + self.test_file["Position"].astype(str)
		valid_error_keys = set(self.ctrl_file.keys()) & set(variant_locations)
		return self.test_file[variant_locations.isin(valid_error_keys)]
	...
	def process_variant(self, variant):
		...
		alphas = self.ctrl_file[variant.location]
		alpha_mapping = {
			'A':0,
			'C':1,
			'G':2,
		...
		## Define varaint information
		loc = variant.location
		chrom = loc.rsplit('_', 1)[0]
		pos = loc.rsplit('_', 1)[1]
		variant_type = 'SNP'
		ref = variant.ref
		alt = variant.alt
		...
		return [chrom, pos, variant_type, ref, alt, alt_count, depth, vaf, p_value, info_column]
...
# emp_pv_df.to_csv(f"/Users/dunmi18p/Desktop/cfDNA_beta_conf/{test_file.split('/')[-1]}_empirical_p_values.tsv", sep="\t", index=False)

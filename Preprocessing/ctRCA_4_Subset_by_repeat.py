...
def parse_args():
	...
	parser.add_argument('--min_repeat', '-r', type=int, action='store', default=2,
	                    help='Minimum repeat count for filtered reads. Defaults to 2')
	parser.add_argument('--split', action='store_true', default=False,
	                    help='Turns on split mode. This will split reads into multiple files based upon repeat count')
	parser.add_argument('--split_min', type=int, action='store', default=0,
	                    help='Integer. Sets the lower bound for splitting. Split mode only. Defaults to 0')
	parser.add_argument('--split_max', type=int, action='store', default=10,
	                    help='Integer. Sets the upper bound for splitting. Split mode only. Defaults to 10')
...
def repeat_cutoff(args, file_path, filename):
	batch_size = 10000
	batch = []
	...
	with open(f"{args.output}{filename}_{str(args.min_repeat)}Plus_repeats.fasta", "w") as f:
		for i, (record_id, sequence) in enumerate(fasta_file.items()):
			...
			repeat_count = float(id_components[3])
			if repeat_count < args.min_repeat:
				continue
...
def split_files(args, file_path, filename):
	...
	repeat_range = range(args.split_min, args.split_max + 1, 1)
	split_repeat_dict = {key: [] for key in repeat_range}
	...
	for i, (record_id, sequence) in enumerate(fasta_file.items()):
		...
		id_components = record_id.split('_')
		repeat_num = float(id_components[3])
		
		if repeat_num not in split_repeat_dict.keys():
			continue
		split_repeat_dict[repeat_num].append(f">{record_id}\n{sequence}\n")
	
	for key, value in split_repeat_dict.items():
		with open(f"{args.output}{filename}_{key}_repeats.fasta", "w") as f:
			f.writelines(value)
...
def main(args):
	...
	print(f"----------------------------------------------------------\n"
	      f"Subsetting Consensus called reads by repeat count\n"
	      f"----------------------------------------------------------\n"
	...
	for file in file_list:
		...
		if args.split:
			...
			repeat_cutoff(args, file_path, file)
			print('')
	print('\n')
